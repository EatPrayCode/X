...
import { appSettingsStateFirebaseBellary, appSettingsStateNull } from '../core/auth/components/choose-app-settings-modal/choose-app-settings-modal.component';
...
export class StateService {
  ...
  saveUserSettingsToFirebase(payload: any) {
    return this.dataService.saveUserSettingsToFirebase(this.appSettingsSubject.value);
  }
    ...
    return auth$.pipe(
      ...
      switchMap((user: appSettingsState) => {
        if (user && user.isLoggedIn) {
          return this.getUserFirebaseSettings(user).pipe(tap((locationSettingsResponse: appSettingsState) => {
