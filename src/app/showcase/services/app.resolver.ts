...
export class AppResolver implements Resolve<any> {
  ...
  public promptAppSettings(): Observable<any> {
    ...
    return this.dialog.open(ChooseAppSettingsModalComponent, modalSettings).afterClosed();
  ...
  resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<any> {
    return this.stateService.getAppSettings().pipe(
      ...
      switchMap(appSettings => !appSettings || !(appSettings.country) ? this.promptAppSettings() : of(appSettings))
    );
  }
}
