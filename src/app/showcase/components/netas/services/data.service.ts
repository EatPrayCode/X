...
export class DataService {
	...
	getAllGroups() {
		return this.afs.collection('/states').snapshotChanges().pipe(
			map((changes: any) => {
	...
	getDistrictsByStateId(gid: string) {
		return this.afs.collection('states/' + gid + "/districts/")
			.snapshotChanges()
			.pipe(
				map((changes: any) => {
					return changes.map((a: { payload: { doc: { data: () => any; id: any; }; }; }) => {
	...
	getGroupByGroupId(id: string) {
		return this.afs.doc('states/' + id).ref.get().then((doc: { exists: any; data: () => any; }) => {
			if (doc.exists) {
	...
	getConstituenciesByDistrictID(stateId: string, districtId: any) {
		return this.afs.collection('states/' + stateId + '/districts/' + districtId + '/constituencies/').snapshotChanges().pipe(
			map((changes: any) => {
				return changes.map((a: { payload: { doc: { data: () => any; id: any; }; }; }) => {
					const data = a.payload.doc.data();
					(<any>data).id = a.payload.doc.id;
	...
	getVillagesByConstituencyID(stateId: string, districtId: any, constituencyId: any) {
		return this.afs.collection('states/' + stateId + '/districts/' + districtId + '/constituencies/' + constituencyId + '/villages/').snapshotChanges().pipe(
			map((changes: any) => {
				return changes.map((a: { payload: { doc: { data: () => any; id: any; }; }; }) => {
					const data = a.payload.doc.data();
					(<any>data).id = a.payload.doc.id;
	...
	addConstituencyWithStateIdDistrictId(gid: string, districtId: any, constituency: any) {
		return this.afs.collection('states/' + gid + '/districts/' + districtId + '/constituencies/').add(constituency);
	}

	addVillageConstituencyIdWithStateIdDistrictId(gid: string, districtId: any, constituencyId: any, village: any) {
		return this.afs.collection('states/' + gid + '/districts/' + districtId + '/constituencies/' + constituencyId + '/villages/').add(village);
	}
